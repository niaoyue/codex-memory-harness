...
- SubAgent 不应该共享同一个粗粒度规则包，而应按自己处理的子项目加载规则；复杂/应用级/多阶段实现即使只命中单个 route，也应默认生成可供主 Agent 调用 Codex SubAgent 的 dispatch plan。
...
- 验证只运行自己项目相关 profile。
- checkpoint 必须包含 `dispatch_id`、`binding_id`、`subagent_id`、`project_id`、`domain`、`assigned_scope` 和 touched paths。
...
当前状态：已完成 route binding 生成、scope guard、coordinator summary 和 dispatch plan 生成的最小运行时；`before_task` 会写入 route plan/bindings、`subagent_runtime` 决策，并在显式、route policy、复杂/应用级任务、xhigh review gate 或通用自动判断命中时写入 `subagent_dispatch_plan.host_spawn_requests`。`route policy` 可从 `.codex/harness/project_profile.json` 或 `.codex/harness/workspace-routing.json` 的 `subagent_runtime_policy` 注入 route plan，让单项目正式 implementation 任务也能持久化授权 Codex SubAgent 派发；通用 planner 会按 `task_intent`、`task_type`、`risk_level`、route 数量、scope 大小和复杂度决定普通小修是否留在主 Agent、功能/系统/发布任务是否派发 `Implementation Specialist`，以及是否额外加入 `Route Review Specialist`；`host_spawn_requests.agent_type` 会直接映射到 `Implementation Specialist`、`Workspace Coordinator`、`Route Review Specialist` 或 `XHigh Review Runner`，并携带 `standing_user_authorization=true`。`after_tool` 会按 touched paths 重算 route、记录 route-bound/SubAgent-style artifact 并执行 scope guard；`before_response` 会输出 routing review 和 runtime decision。插件不内建独立 agent 子进程调度；用户明确选择、项目 policy 授权或通用 planner 判定需要派发时，由主 agent 消费 `host_spawn_requests` 调用当前 Codex SubAgent 能力。
