...
- 支持把 Codex CLI xhigh review gate 委托给 XHigh Review Runner SubAgent 并行执行，按 stdout/stderr 和状态进度观察，不再套固定总时长；最终审核语义来自候选提交后的 `codex xhigh review --base HEAD~1`。
...
| 发布/提交后 review gate | 聚合验证、scope guard、敏感扫描、诊断日志 release gate、candidate commit、`codex xhigh review --base HEAD~1` | 部分实现 | 验证聚合、scope guard、敏感扫描、基础诊断 release gate、review preflight、commit diff fingerprint、ledger、runner 恢复和 slice planner 已有；仍不是覆盖渠道包、热更、平台配置和回滚材料的完整发布平台 |
...
7. 修改完成后自动聚合验证。
8. 代码变更先通过 review preflight 和验证，再创建本地 candidate commit；随后通过 `codex xhigh review --base HEAD~1` 审核本次提交。大 diff 或长耗时审查优先由 XHigh Review Runner SubAgent 作为命令执行器并行运行该 gate，持续有输出时不按固定总时长失败，也不得再套任何外层总时长。
...
flowchart TD
  ...
  AG0 --> AG1["代码审核 gate: codex xhigh review --base HEAD~1"]
...
9. 验证通过后先创建本地 candidate commit，并记录 `HEAD~1..HEAD` 的 commit diff fingerprint。
10. 运行 `codex xhigh review --base HEAD~1`；大 diff 或长耗时审查优先派发 XHigh Review Runner SubAgent 执行该命令，按 stdout/stderr 和状态进度观察，不再套固定总时长。SubAgent 自行审查只作为专题辅助，不替代 Codex CLI xhigh review gate。
11. 如果 review findings 指向本次提交，先修复，再创建新的本地提交或在未 push 前重做候选提交，然后重新 review 最新提交；若工作树包含用户无关改动，只提交本轮相关文件。
...
XHigh Review Runner SubAgent 只负责运行 `codex xhigh review --base HEAD~1` 或明确的降级命令，不修改文件、不提交，也不替代专题 Reviewer。
