...
模型不会直接把这句话当作一个整体处理，而是会先由 tokenizer 切成若干 token。中文里一个汉字可能接近一个 token，英文里一个单词可能被切成一个或多个 token，代码里的符号、缩进、路径、变量名也会占 token。

例如 `苹果` 和 `apple` 语义相近，但通常不会被 tokenizer 切成同一个 token。它们会先变成不同 token id，再在模型内部变成向量。模型能把它们联系起来，是训练后学到的语义关系，不是 token 天然相同。
...
很多 RAG 系统会使用向量数据库。向量数据库会把文本 chunk 转换成 embedding，用语义相似度检索相关内容。
...
| 本轮到底应该带哪些材料进模型 | context pack | 需要在 token 预算内装配证据 |
...
这些问题依赖结构化任务状态，而不是只靠语义相似度。即使 RAG 能召回相关文字，也不能自动证明当前工作区已经修改、测试或 review 通过。
...
当前它不提供真实 SubAgent 自动执行器、向量数据库或 eval replay 平台，也不会按 workspace route 自动把子项目事实分层写入长期 memory。
...
```text
顶部：一句话说明
第一屏：Token / token id / 向量表示 / LLM / Agent / Memory 的关系图
第二屏：Agent 请求和返回拆解
...
安全边界也适用于 RAG 和向量索引。敏感文本一旦进入索引，即使原文删除，索引、缓存和摘要仍可能残留。
