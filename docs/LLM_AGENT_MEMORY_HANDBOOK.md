...
它不从命令开始，而是先解释大模型、Token、Agent、Codex 工作流和记忆系统。读者理解这些基础后，再看 Codex Memory Harness 的安装、使用和验证闭环，会更容易判断它解决的到底是什么问题。
...
Token 是大模型处理文本时使用的基本单位。它不完全等于一个字、一个词或一个字符。
...
模型不会直接把这句话当作一个整体处理，而是会切成若干 token。中文里一个汉字可能接近一个 token，英文里一个单词可能被切成一个或多个 token，代码里的符号、缩进、路径、变量名也会占 token。

可以把 token 理解成模型的输入和输出预算单位：
...
这些内容都会进入模型上下文。上下文越长，消耗的 token 越多。即使模型支持很长上下文，也不能无限塞入全部仓库、全部日志和全部历史。
...
```text
输入：一组消息、规则、上下文和用户问题
输出：下一段最可能有用的文本，或一个需要执行的工具调用请求
```

它不是在数据库里查答案，也不是天然知道你的本地项目状态。它根据当前上下文进行推理和生成。
...
普通 LLM 主要回答问题。Agent 则会把 LLM 接到外部工具和任务循环上。
...
模型拿到这些内容后，会决定下一步是直接回答，还是请求执行工具。
...
| 中间状态说明 | 告诉用户当前正在分析、编辑或验证 |
| 最终回答 | 说明结果、验证、风险和后续建议 |

工具调用本身不是模型直接在系统里执行命令，而是模型向宿主发出结构化请求，宿主执行后把结果再交还给模型。
...
所以成熟 Agent 需要一个外部状态系统，记录任务目标、约束、工作集、工具事件、验证结果和下一步。
...
这说明 Codex 的价值不只是“生成代码”，而是把模型推理、工具调用和工程验证组织成一个闭环。

### 5.2 Codex 的请求上下文大概长什么样

一次 Codex 推理前，上下文可能类似这样：
...
### 5.3 Codex 的 context pack 是什么

context pack 可以理解成“本次推理要带给模型的一包上下文”。它不是把所有历史都带上，而是把当前最相关的材料按优先级装配。
...
当 Codex 调用工具后，宿主会把工具结果重新放回上下文。例如：
...
模型不会自动“知道”工具执行结果，必须由宿主把结果作为新的消息交回模型。然后模型才能继续判断：
...
很多 RAG 系统会使用向量数据库。向量数据库会把文本转换成 embedding，用语义相似度检索相关内容。
...
| 官方 Codex 自动记住的个人偏好是什么 | 官方 Codex Memories | 由官方机制管理 |
...
```text
工程记忆负责“当前任务怎么可靠推进”，RAG 负责“历史知识怎么召回”。
```
...
注意这里记录的是摘要，不应该把完整日志、密钥、令牌或大量原始输出写入记忆。
...
这样最终回答不是凭印象写，而是基于结构化任务状态写。

### 9.4 on_task_complete：任务完成后

任务完成后，Harness 可以写入 summary：
...
如果这些结论稳定、脱敏、适合团队共享，可以再提升到 `.codex/shared`。
...
```text
顶部：一句话说明
第一屏：Token / LLM / Agent / Memory 的关系图
第二屏：Agent 请求和返回拆解
第三屏：临时记忆、工程记忆、RAG 对比表
第四屏：Codex Memory Harness 3W1H
第五屏：一次任务的生命周期
第六屏：安装和日常使用命令
第七屏：安全边界和常见误区
底部：链接到 USER_GUIDE、MEMORY_LAYERING、MEMORY_RETRIEVAL_STRATEGY
```
...
| 核对点 | 应该讲清楚什么 | 当前页面位置 |
|---|---|---|
| Token | Token 是预算，不是简单字数；上下文、工具结果和最终回答都会占 token | 第 2 节 |
| LLM | LLM 根据当前上下文生成，不天然知道本机项目状态 | 第 3 节 |
