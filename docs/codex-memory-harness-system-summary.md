...
| 10 | Bundled skills 层 | vendor openai/skills curated 子集，安装到 `$CODEX_HOME/skills` |
...
```text
$CODEX_HOME/skills/security-best-practices
$CODEX_HOME/skills/security-threat-model
$CODEX_HOME/skills/cli-creator
$CODEX_HOME/skills/migrate-to-codex
$CODEX_HOME/skills/gh-fix-ci
$CODEX_HOME/skills/gh-address-comments
```
...
- 默认从发布包内的 `plugins/codex-memory/skills/openai-curated` 离线复制 bundled skills 到 `$CODEX_HOME/skills`；目标技能已存在时跳过，不覆盖用户版本。
- 可用 `--skip-skills` 或 `-SkipSkills` 跳过 bundled skills 安装。
- 替换旧普通目录时会备份，不直接删除。
- profile 和 AGENTS 采用标记块写入，降低误改用户内容的风险。
- 旧的未标记全局 AGENTS 规则会保留，不强行覆盖。
- 卸载默认不删除 `$CODEX_HOME/skills`，避免误删用户自行安装或修改的技能。
...
打包会包含 `plugins/codex-memory/skills/openai-curated/` 和 `plugins/codex-memory/skills/bundled-skills.json`，以便目标机器安装时不依赖 GitHub 网络。
