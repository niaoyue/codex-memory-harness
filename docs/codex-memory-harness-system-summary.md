...
| XHigh Review Runner | 并行执行固定 base 的 `codex xhigh review --base <candidate-base>` 审核完整 candidate 范围，必要时降级到 `codex-raw -- review -c model_reasoning_effort="xhigh" --base <candidate-base>`，按 stdout/stderr 和状态进度观察且不套固定总时长，回传退出状态和 findings |
