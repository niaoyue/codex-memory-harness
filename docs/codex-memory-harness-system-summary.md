...
| 10 | Bundled skills 层 | 按 manifest vendor 所有可用治理技能，安装到 `~/.agents/skills`，并避开同名 `.system` 技能 |
...
当前 manifest 覆盖 security、GitHub、CLI、迁移、release gate、需求澄清、接口设计、TDD、提交、review、PRD、重构、文档、图像、OpenAI docs、plugin/skill 创建等可用技能；安装时不联网下载。目标技能目录已存在且内容不同会先备份到 `~/.agents/skills/.codex-memory-backups/`，再刷新为当前包版本；若 Codex 已提供同名 `.system` 技能，则跳过用户目录副本；若旧版 `~/.codex/skills/<skill-name>` 中还有同名副本，则移动到 `.codex-memory-backups/`，保留内容但退出发现路径。
...
- 默认从发布包内的 `plugins/codex-memory/skills/openai-curated` 和 `plugins/codex-memory/skills/local` 离线安装 bundled skills 到 `~/.agents/skills`；目标技能目录已存在且内容不同会先备份到 `.codex-memory-backups`，再刷新为当前包版本；同名 `.system` 技能优先，旧版 `~/.codex/skills` 同名副本会备份移出发现路径。
...
- Bundled skills 固定到 manifest 记录的 upstream commit；更新技能需要重新 vendor 并更新 `skills/bundled-skills.json`，安装/更新时会把目标机器上的同名旧技能备份后刷新，并把 legacy/system 发现路径中的重复副本备份移出。
