...
| XHigh Review Runner | 并行执行 `codex xhigh review --base HEAD~1` 审核最新 candidate commit，必要时降级到 `codex-raw -- review -c model_reasoning_effort="xhigh" --base HEAD~1`，按 stdout/stderr 和状态进度观察且不套固定总时长，回传退出状态和 findings |
