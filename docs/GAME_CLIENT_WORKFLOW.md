...
| `release-check` | 聚合构建、包体、热更、版本号、渠道配置和回滚策略检查 |

但在多项目 workspace 下，更推荐 `codex workspace ...` 自动识别后路由到 `game_client`。当前未实现该命令；目前只落地文档和路线。
...
3. 运行层：继续使用 `codex harness verify run --profile <profile>`，等多个业务项目都出现同类需求后，优先实现 `codex workspace ...` 自动路由，再决定是否保留单项目 `codex game-client ...` 调试入口。
