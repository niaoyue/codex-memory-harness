...
需求不清时不能由 AI 猜测。运行时会通过 `requirements_gate` 区分 bugfix、小优化、功能 story、系统改动、发布 gate、文档任务和技术任务；`feature_story`、`system_change`、`release_gate` 缺少关键需求来源或验收材料时会要求澄清。完整规则见：
