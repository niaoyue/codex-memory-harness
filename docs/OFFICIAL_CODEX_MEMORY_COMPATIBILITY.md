...
Doctor 还会检查官方 Codex native 接入相关状态：

- `features.codex_hooks` 是否启用。
- 插件 `hooks.json` 是否覆盖 `UserPromptSubmit`、`PostToolUse` 和 `Stop`。
- `sandbox_mode` 与 `approval_policy` 是否处于全权限无审批组合。
- 是否存在 `AGENTS.override.md`，从而可能覆盖全局 `AGENTS.md` 规则。
- `mcp_servers` 中是否显式配置 `codex-memory`。

`doctor` 和 `check-install` 默认只读，只给出建议；`install` 和 `update` 会自动修复必要的低风险官方配置，目前会确保 `$CODEX_HOME/config.toml` 中存在 `[features] codex_hooks = true`。
