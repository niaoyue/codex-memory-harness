...
当前 Codex Memory Harness 没有实现并发 SubAgent 调度器，也不会自动创建多个 agent 进程。
...
| Reviewer | 从回归、边界、可维护性角度审查 | review findings |
...
3. Reviewer 审查 diff。
4. Implementer 修复 review findings。
5. Verifier 运行验证。
6. Documentation Maintainer 补齐文档。
7. Security Reviewer 做敏感信息和边界扫描。
8. Release Manager 准备提交说明。
