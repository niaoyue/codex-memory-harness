...
| XHigh Review Runner | 作为并行命令执行器运行 `codex xhigh review --uncommitted`，必要时降级到 `codex-raw xhigh review --uncommitted` | xhigh review 结果、失败原因 |
...
- 需要多专题辅助审查，并且最终仍由 Codex CLI 的 `codex xhigh review --uncommitted` 做代码审核 gate。
...
- reviewer 不直接重写大块代码，除非被明确授权；代码变更的最终审核优先使用 Codex CLI 的 `codex xhigh review --uncommitted`。
...
4. 主流程运行 `codex xhigh review --uncommitted` 作为最终代码审核 gate；大 diff 时优先派发 XHigh Review Runner SubAgent 并行执行该命令。
...
- XHigh Review Runner 可在 diff 稳定后并行运行，建议等待预算按 60 分钟设置；它只运行 Codex CLI review，不替代专题 Reviewer。
...
在仓库根目录运行 codex xhigh review --uncommitted。
如果 wrapper 不稳定，降级运行 codex-raw xhigh review --uncommitted。
等待预算按 60 分钟处理，返回退出码、是否有 findings、关键 findings 摘要和降级原因。
```
...
如果 xhigh review 返回 findings，主 agent 修复后必须重新运行同一 gate，直到没有新的阻断问题。SubAgent 超时或 wrapper 失败时，必须记录失败原因，并由主 agent 直接运行 `codex-raw xhigh review --uncommitted` 或原生 Codex review 命令兜底。
...
- 如果宿主环境支持 SubAgent，主 agent 可以派发 XHigh Review Runner 执行 `codex xhigh review --uncommitted`；这属于使用宿主能力，不代表本仓库内建了自动执行器。
