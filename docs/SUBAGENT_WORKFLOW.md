...
| XHigh Review Runner | 作为并行命令执行器运行 `codex xhigh review --uncommitted`，必要时降级到 `codex-raw -- review -c model_reasoning_effort="xhigh" --uncommitted` | xhigh review 结果、失败原因 |
...
9. 如果最终 review gate 无阻断 findings 且验证通过，Release Manager 必须在本地创建一个 git commit 记录当前版本。
...
如果 wrapper 不稳定，降级运行 codex-raw -- review -c model_reasoning_effort="xhigh" --uncommitted。
...
- 当 planner 判定实现任务需要旁路审查时，dispatch plan 会在 route specialist 之外加入 `Route Review Specialist`。它只做限定 scope 的风险和测试覆盖审查，不替代最终 `codex xhigh review --uncommitted` gate。
