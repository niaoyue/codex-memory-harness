...
| XHigh Review Runner | 作为并行命令执行器运行固定 base 的 `codex xhigh review --base <candidate-base>`，首次单提交候选可用 `HEAD~1`，必要时降级到 `codex-raw -- review -c model_reasoning_effort="xhigh" --base <candidate-base>` | xhigh review 结果、失败原因 |
...
- 需要多专题辅助审查，并且最终仍由 Codex CLI 的 `codex xhigh review --base <candidate-base>` 审核完整 candidate 范围。
...
- reviewer 不直接重写大块代码，除非被明确授权；代码变更的最终审核优先使用 Codex CLI 的 `codex xhigh review --base <candidate-base>` 审核完整 candidate 范围。
...
4. Verifier 运行验证。
5. Release Manager 创建只包含本轮相关文件的本地 candidate commit，并记录固定 `<candidate-base>`。
6. 主流程运行 `codex xhigh review --base <candidate-base>` 审核从原始候选基线到最新 HEAD 的完整变更；大 diff 时优先派发 XHigh Review Runner SubAgent 并行执行该命令。
...
在仓库根目录运行 codex xhigh review --base <candidate-base>。
如果 wrapper 不稳定，降级运行 codex-raw -- review -c model_reasoning_effort="xhigh" --base <candidate-base>。
...
如果 xhigh review 返回 findings，主 agent 修复后必须创建新的本地提交或在未 push 前重做候选提交，再用同一个 `<candidate-base>` 重新运行同一 gate，直到没有新的阻断问题。SubAgent 发生 idle timeout、wrapper 失败或宿主不支持 SubAgent 时，必须记录失败原因，并由主 agent 直接运行 `codex-raw -- review -c model_reasoning_effort="xhigh" --base <candidate-base>` 或原生 Codex review 命令兜底。

candidate commit 只应包含本轮相关文件；如果工作树里混有用户无关改动，必须先隔离提交范围，无法安全隔离时在最终答复说明未提交原因。固定 base 的候选范围 review 无新阻断问题后才允许 push。
...
- 当 planner 判定实现任务需要旁路审查时，dispatch plan 会在 route specialist 之外加入 `Route Review Specialist`。它只做限定 scope 的风险和测试覆盖审查，不替代最终固定 base 的 `codex xhigh review --base <candidate-base>` gate。
- 如果宿主没有 SubAgent 工具、调用失败或 scope 冲突，主 agent 必须记录降级原因，并按同一个 dispatch plan 串行执行或回到普通主 Agent 流程。
- 如果宿主环境支持 SubAgent，主 agent 可以派发 XHigh Review Runner 执行 `codex xhigh review --base <candidate-base>`；这属于使用宿主能力，不代表本仓库内建了自动执行器。
