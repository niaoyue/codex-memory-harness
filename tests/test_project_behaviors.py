...
class BuildReleaseTests(unittest.TestCase):
    def test_release_excludes_runtime_codex_and_generated_files(self) -> None:
        ...
        self.assertIn("uninstall.bat", manifest["uninstall"])
        self.assertIn("uninstall.ps1", manifest["uninstall"])
        self.assertIn("uninstall.sh", manifest["uninstall"])
        self.assertIn("version_manager.py", manifest["version_management"])
        self.assertIn("uninstall.bat", names)
        self.assertIn("uninstall.sh", names)
        self.assertIn("uninstall.ps1", names)
        self.assertIn("scripts/version_manager.py", names)
...
class BootstrapTests(unittest.TestCase):
    ...
    def test_doctor_is_not_ready_when_home_plugin_points_elsewhere(self) -> None:
        ...
        self.assertTrue(any("默认会让 home plugin 更新" in item for item in result["recommendations"]))
