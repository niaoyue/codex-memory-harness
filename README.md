...
- 启动自检：由 Codex Memory Harness 检查插件、目标项目 `.codex` 接入配置、官方 hooks、sandbox/approval 和推荐环境变量；这些检查结果不代表目标项目自身已经实现 memory、harness 或 OpenSpec 业务能力。
- Harness 生命周期：用 `start`、`checkpoint`、`complete` 记录任务过程。
- 验证回写：运行项目配置化验证，并把结果写回 harness artifact。
- SubAgent 等待策略：所有 SubAgent 都没有固定总时长；Codex SubAgent 观察窗口或命令执行器 timeout 只是单次观察窗口，窗口到期后继续观察，不得仅因窗口到期中断或判失败。
...
- OpenSpec specs：持久规格与 change contract 默认遵循 OpenSpec `spec-driven` 结构；新变更写入 `openspec/changes/<change-id>/proposal.md`、`specs/<capability>/spec.md`、`design.md`、`tasks.md`，稳定行为规格写入 `openspec/specs/<capability>/spec.md`，Harness 衔接写入同一 change 下的 `harness.json` / `harness.md`；启动自检、新项目初始化和维护刷新时，在目标项目 memory/harness init 之后默认执行 `codex openspec upstream sync --version 1.3.1`，再执行 `codex openspec upstream verify`，把官方 OpenSpec schema/templates/license/package metadata 从 `@fission-ai/openspec` 同步到目标项目的 `openspec/upstream/openspec/` 并用 manifest 哈希校验。该流程是 Harness 提供的接入/校验能力，不应写成目标项目已有实现。
- 未完成 Task 进度汇总：显式请求未完成项时，`before_response` 可返回结构化 `unfinished_task_summary` 和 Markdown，字段包含状态、最近 checkpoint 或更新时间、已完成/剩余验收、阻塞点、下一步和证据来源；本仓库默认优先读取 `.codex/harness/backlog/tasks.md`，迁移期再回退旧 `docs/codex-memory-plugin-task-list.md`。
...
- 历史记忆挖掘 runtime：自动记录脱敏事件；通过 `codex memory mine run` 显式生成候选、`codex memory candidates ...` 治理候选，并把 accepted preferences 注入 context pack。低频自动触发挖掘仍是后续路线。
...
- `subagent_receipts.py` 支持 Codex SubAgent 执行回执导入和 integration readiness report；插件不自行启动独立 agent 子进程。
- `codex workspace schedule` 标准化 SubAgent dispatch plan，包含 `host_spawn_requests`，供主 Agent 在用户显式选择、项目持久化 `subagent_runtime_policy` 授权、通用 planner 判定功能/系统/发布/复杂任务需要派发，或 xhigh review gate 需要并行执行时调用当前 Codex 会话提供的 SubAgent 能力；普通低风险小修仍可保持主 Agent 串行。安装后的全局 AGENTS 规则会明确要求：当 `subagent_runtime.recommended=true`、`host_dispatch_allowed=true` 且存在 `host_spawn_requests` 时，MainAgent 必须消费 request 并调用 `spawn_agent` 或等价宿主 SubAgent 工具；若降级为串行，必须记录 `actual_subagents=0`、`downgrade_reason` 和未执行的 `dispatch_id`。
...
- Release manifest gate：`release_profile_gate.py --manifest-file` 对渠道/平台/产物/回滚/evidence manifest 做本地阻断检查，包含 artifact kind/platform/sha256/size 和 evidence report_path 校验，但不执行平台构建。
...
- 不包含仓库内建的 SubAgent 子进程执行器；`workspace schedule` 只生成 dispatch plan 和 `host_spawn_requests`，不自行启动子进程或远程 agent。用户明确选择 SubAgent、任务属于复杂/应用级/多阶段实现，或项目 policy 授权时，由主 Agent 消费 `subagent_dispatch_plan.host_spawn_requests` 并映射到当前 Codex SubAgent 能力。
- 不包含发布级完整验证平台；当前 release gate 是可阻断的基础静态检查和 verification aggregation，不覆盖所有平台构建配置。
- 不包含完整的宿主级 session-worktree 自动绑定 runtime；当前已有最小 `codex workspace write-guard`、binding registry、worktree allocator 和 `hook_runner` 的 `before_first_write` 软事件，能在 dirty primary checkout 或已有 active write lease 时返回 managed worktree 的 `effective_cwd`。后续仍需接入宿主原生写入前强制拦截和多 session 自动合并。
...
`codex` 正常启动时也会由 Codex Memory Harness 自动检查并初始化缺失的项目接入配置；手动执行 `codex memory init` 主要用于排查或提前准备项目。完成目标项目 memory/harness 接入初始化后，Harness 默认继续刷新并校验 OpenSpec 上游快照：
...
这不是只有项目选择 OpenSpec specs 时才执行的可选步骤，而是 Harness 启动和新项目接入初始化的默认基线，用于固定官方 schema/templates/license/package metadata。它只说明 Harness 已在目标项目内初始化/校验所需接入材料，不说明目标项目自身已经实现 memory、harness 或 OpenSpec 业务能力。
...
| OpenSpec 规格层 | `<项目根目录>/openspec` | 可提交 | OpenSpec `spec-driven` specs、change contracts、delta specs、官方 upstream snapshot 和 Harness binding extension |
| Harness backlog 层 | `<项目根目录>/.codex/harness/backlog` | 可提交，不随 release zip 分发 | Harness 自身任务清单、迁移计划和未完成项进度来源，不作为正式 spec 层 |
| 项目共享层 | `<项目根目录>/.codex/shared` | 可提交，需审查 | 团队确认的项目事实、决策、流程、路由摘要 |

当前已经实现用户全局层、项目私有层和项目共享层初始化模板；正式规格层已对齐 OpenSpec，`.codex/specs` 不再作为新规格目录。项目共享层只放可审查的 Markdown 和索引，不能直接用 raw `.codex/memories` 代替。官方 Codex Memories 只作为个人自动记忆来源，不作为团队事实来源。
...
`sync --version 1.3.1` 从官方 npm 包 `@fission-ai/openspec` 同步 `LICENSE`、`README.md`、`package.json` 和 `schemas/spec-driven/**` 到 `openspec/upstream/openspec/`，并写入 `manifest.json`；随后必须运行 `verify` 校验本地 snapshot。Harness 自己的执行、记忆、验证和 review gate 逻辑仍留在本项目 adapter 中，不修改 upstream snapshot。
...
`openspec/` 是源码仓库内可提交的正式规格目录，包含 OpenSpec specs/changes 和 pinned upstream snapshot；当前 release zip 仍排除根目录 `.codex/`，避免把 Harness backlog、项目治理材料和本机运行态一起分发到用户安装包。
