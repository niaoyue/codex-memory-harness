...
- Review gate 优化规划：保留 `codex xhigh review --uncommitted` 最终语义，通过 preflight、diff fingerprint、runner 恢复、findings ledger 和 slice planner 降低长耗时与重复失败。
...
- 不包含已实现的自动历史挖掘 runtime；当前已经落地设计方案，后续会按事件账本、候选挖掘、自动提升、上下文注入和治理命令逐步实现。
- 不包含已实现的 review gate 优化 runtime；当前已经落地设计方案，后续会按 preflight、diff fingerprint、review ledger、runner 恢复和 slice planner 逐步实现。
- 不包含已实现的 session-worktree 自动绑定 runtime；当前已经落地设计方案，后续会按 binding registry、worktree allocator、lifecycle 集成、stale cleanup 和多 session 协作逐步实现。
