...
- 未完成 Task 进度汇总：显式请求未完成项时，`before_response` 可返回结构化 `unfinished_task_summary` 和 Markdown，字段包含状态、最近 checkpoint 或更新时间、已完成/剩余验收、阻塞点、下一步和证据来源；本仓库默认优先读取 `.codex/specs/backlog-governance/tasks.md`，迁移期再回退旧 `docs/codex-memory-plugin-task-list.md`。
