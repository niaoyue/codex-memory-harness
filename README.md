...
同步或验证官方 OpenSpec 上游核心 schema/templates：
...
该命令从官方 npm 包 `@fission-ai/openspec` 同步 `LICENSE`、`README.md`、`package.json` 和 `schemas/spec-driven/**` 到 `openspec/upstream/openspec/`，并写入 `manifest.json`。Harness 自己的执行、记忆、验证和 review gate 逻辑仍留在本项目 adapter 中，不修改 upstream snapshot。
