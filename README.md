...
- `codex workspace schedule` 标准化 SubAgent dispatch plan，包含 `host_spawn_requests`，供主 Agent 在用户显式选择、项目持久化 `subagent_runtime_policy` 授权、通用 planner 判定功能/系统/发布/复杂任务需要派发，或 xhigh review gate 需要并行执行时调用当前 Codex 会话提供的 SubAgent 能力；普通低风险小修仍可保持主 Agent 串行。
