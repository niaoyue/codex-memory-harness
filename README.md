...
- 不包含仓库内建的 SubAgent 子进程执行器；`workspace schedule` 只生成调度计划和宿主调用请求，不自行启动子进程或远程 agent。用户明确选择 SubAgent、任务属于复杂/应用级/多阶段实现，或项目 policy 授权时，由主 Agent 消费 `subagent_dispatch_plan.host_spawn_requests` 调用 Codex SubAgent。
