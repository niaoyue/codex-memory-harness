...
- OpenSpec specs：持久规格与 change contract 默认遵循 OpenSpec `spec-driven` 结构；新变更写入 `openspec/changes/<change-id>/proposal.md`、`specs/<capability>/spec.md`、`design.md`、`tasks.md`，稳定行为规格写入 `openspec/specs/<capability>/spec.md`，Harness 衔接写入同一 change 下的 `harness.json` / `harness.md`；启动自检、新项目初始化和维护刷新时，在项目 memory/harness init 之后默认执行 `codex openspec upstream sync --version 1.3.1`，再执行 `codex openspec upstream verify`，把官方 OpenSpec schema/templates/license/package metadata 从 `@fission-ai/openspec` 同步到 `openspec/upstream/openspec/` 并用 manifest 哈希校验。
...
`codex` 正常启动时也会自动检查并初始化缺失的项目配置；手动执行 `codex memory init` 主要用于排查或提前准备项目。完成项目 memory/harness 初始化后，默认继续刷新并校验 OpenSpec 上游快照：
...
这不是只有项目选择 OpenSpec specs 时才执行的可选步骤，而是启动和新项目初始化的默认基线，用于固定官方 schema/templates/license/package metadata。
...
同步或验证官方 OpenSpec 上游核心 schema/templates。默认初始化和刷新顺序是先同步指定版本，再校验 manifest；`status` 只用于前后状态检查：
