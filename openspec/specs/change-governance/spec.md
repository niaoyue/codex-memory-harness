...
#### Scenario: OpenSpec upstream assets are synchronized locally

- GIVEN Harness needs local OpenSpec schema, template, license, or package
  metadata files
- WHEN those files are used for validation, distribution, or adapter evidence
- THEN they MUST come from the pinned official `@fission-ai/openspec` package
  snapshot under `openspec/upstream/openspec/`
- AND the snapshot MUST include a manifest with package version, integrity,
  file hashes, license, telemetry policy, and an update command
