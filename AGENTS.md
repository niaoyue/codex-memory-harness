...
- 禁止危险命令、泄露密钥、上传敏感信息。
- 代码审核优先使用 `codex xhigh review --uncommitted` 作为最终 review gate；大 diff 可让 SubAgent 作为并行命令执行器运行该 gate。通用 SubAgent reviewer 只做窄范围专题/旁路审查，不替代 Codex CLI xhigh review。
