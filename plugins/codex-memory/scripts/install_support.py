...
- 代码审核采用候选提交后 review：验证通过并确认提交边界后，先在本地创建一个只包含本轮相关文件的 candidate commit，再使用 `codex xhigh review --base HEAD~1` 审核这次提交；大 diff 或长耗时审查优先派发 XHigh Review Runner SubAgent 作为并行命令执行器运行该 gate，等待策略按 stdout/stderr 进度输出观察，不使用固定总时长超时，也不得再用 10 分钟外层总时长包住 runner。runner 遇到模型容量、429、5xx 或超时类基础设施错误时，如果宿主持有仍活跃的 runner session，必须先按类型退避并对同一 session 发送继续指令：容量/429 退避 20 秒且只要 session 活跃就可继续，5xx/超时退避 2 秒且最多续跑一次；只有 session 已关闭、无句柄、不可恢复或本次提交内容已变化时，才重新启动同一个 review gate。通用 SubAgent reviewer 只做限定 scope 的专题/旁路审查。
- 如果 review 本次提交发现阻断问题，必须修复后创建新的本地提交（或在尚未 push 前重做候选提交），再继续用 `codex xhigh review --base HEAD~1` 审核最新提交；直到 review 没有新的阻断问题后才允许 push。若工作树包含用户无关改动，必须只提交本轮相关文件或先说明无法安全提交。
