...
$MemoryServer = Join-Path $PluginRoot "mcp\memory_server.py"
...
if (-not $runtime.Command) {
    Write-Error "Python 3.11 or newer is required to run Codex Memory MCP."
    exit 127
}

$pythonArgs = @($runtime.PrefixArgs) + @("-X", "utf8", $MemoryServer) + @($McpArgs)
