...
| T87 | 24 | 实现严格 Requirements Integrity Gate runtime | 已完成最小 runtime、blocking write enforcement、governance adapter、误判修复与 `requirements_conflict_scanner.py` 显式 spec/implementation conflict 静态扫描；2026-05-13 HarnessTest 已验证 OpenSpec、BMAD planning artifact、verification 和 clean review evidence bundle，`safe_to_archive=true`；真实 BMAD upstream 执行仍需外部工具或用户规划材料 | T84,T86 | doing |
...
> 更新时间：2026-05-13。本节用于满足“未完成 Task 汇总必须带进度”的治理要求；缺少外部证据时明确标记 `unknown`。

### T55 - 实现发布级完整验证平台

- status: doing
- recent_checkpoint_or_update: Step 33 / Step 41 in this task list
- completed_acceptance: release profile/evidence gate、release manifest gate、eval replay 已完成；manifest gate 已本地校验 artifact kind/platform/sha256/size、evidence report_path 和未知 gate status
...
- status: doing
- recent_checkpoint_or_update: 2026-05-13T09:57:14+00:00 HarnessTest dogfood checkpoint
- completed_acceptance: 最小 `write-guard`、`before_first_write` 软事件和 dirty primary checkout 下 `switch_to_effective_cwd` 已验证
...
- status: doing
- recent_checkpoint_or_update: 2026-05-13T09:57:14+00:00 HarnessTest dogfood checkpoint
- completed_acceptance: 最小 runtime、blocking write enforcement、governance adapter、误判修复、spec/implementation conflict scanner 已完成；HarnessTest evidence bundle `safe_to_archive=true`
